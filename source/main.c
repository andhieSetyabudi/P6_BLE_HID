...
#define MOUSE_UPDATE_MS   			(9999u)
#define MOUSE_LEVEL_UPDATE_FREQ 	(10000)

// (period+1) /freq to get 50ms		
#define SENSOR_UPDATE_MS			(4999u)		// timer period - 1
#define SENSOR_UPDATE_FREQ			(100000u)   // in Hz	
...
/**
 * @brief HAL Variable  object
 */
static cyhal_timer_t bas_timer_obj;
static cyhal_timer_t mouse_timer_obj;
static cyhal_timer_t sensor_timer_obj;
...
const cyhal_timer_cfg_t mouse_timer_cfg =
    {
        .compare_value = 0,                    /* Timer compare value, not used */
        .period = MOUSE_UPDATE_MS, 				/* Defines the timer period */
        .direction = CYHAL_TIMER_DIR_UP,       /* Timer counts up */
        .is_compare = false,                   /* Don't use compare mode */
        .is_continuous = true,                 /* Run timer indefinitely */
        .value = 0                             /* Initial value of counter */
...
const cyhal_timer_cfg_t sensor_timer_cfg =
    {
        .compare_value = 0,                    /* Timer compare value, not used */
        .period = SENSOR_UPDATE_MS, 				/* Defines the timer period */
        .direction = CYHAL_TIMER_DIR_UP,       /* Timer counts up */
        .is_compare = false,                   /* Don't use compare mode */
        .is_continuous = true,                 /* Run timer indefinitely */
        .value = 0                             /* Initial value of counter */
...
static void 				  app_bt_mouse_init                     (void);

#define MOUSE_REPORT_SIZE   4
#define MOUSE_REPORT_BUFF_SIZE  10
uint8_t app_hid_mouse_report[MOUSE_REPORT_SIZE];
...
void mouse_timer_callb(void *callback_arg, cyhal_timer_event_t event);
void sensor_timer_callb(void *callback_arg, cyhal_timer_event_t event);
...
int main()
{
    ...
    printf("========Mouse HID Application Start========\r\n");
    printf("================================================\n");
    printf("================================================\n\n");
	...
	rtos_result = xTaskCreate(mouse_task, "Mouse Task", (configMINIMAL_STACK_SIZE * 4),
                                    NULL, (configMAX_PRIORITIES - 3), &mouse_task_handle);
    if(pdPASS == rtos_result)
    {
        printf("Mouse task created successfully\n");
    }
    else
    {
	...
	rtos_result = xTaskCreate(sensor_task, "Sensor Task", (configMINIMAL_STACK_SIZE * 4),
                                    NULL, (configMAX_PRIORITIES - 2), &sensor_task_handle);
    if(pdPASS == rtos_result)
    {
	...
	printf("SPI and sensor initializing \r\n");
	sensor_init();
	printf("Load first dumb data from sensor");
	//sensor_update();
	sensor_print();
                                          ...
                                          wiced_bt_management_evt_data_t *p_event_data)
{
	...
	//wiced_bt_device_address_t local_bda=(wiced_bt_device_address_t)BD_ADDR;
...
// timer for HID Mouse 
	/* Initialize the HAL timer used to count seconds */
    cy_result = cyhal_timer_init(&mouse_timer_obj, NC, NULL);
    if (CY_RSLT_SUCCESS != cy_result)
    {
        printf("mouse timer init failed !\n");
    }
    /* Configure the timer for 5 seconds */
    cyhal_timer_configure(&mouse_timer_obj, &mouse_timer_cfg);
    cy_result = cyhal_timer_set_frequency(&mouse_timer_obj, MOUSE_LEVEL_UPDATE_FREQ);
    if (CY_RSLT_SUCCESS != cy_result)
    {
        printf("mouse timer set freq failed !\n");
    }
 	cyhal_timer_register_callback(&mouse_timer_obj, mouse_timer_callb, NULL);
    cyhal_timer_enable_event(&mouse_timer_obj, CYHAL_TIMER_IRQ_TERMINAL_COUNT, 3, true);
	
// timer for sensor update
	/* Initialize the HAL timer used to count seconds */
    cy_result = cyhal_timer_init(&sensor_timer_obj, NC, NULL);
    if (CY_RSLT_SUCCESS != cy_result)
    {
        printf("sensor timer init failed !\n");
    }
    /* Configure the timer for 5 seconds */
    cyhal_timer_configure(&sensor_timer_obj, &sensor_timer_cfg);
    cy_result = cyhal_timer_set_frequency(&sensor_timer_obj, SENSOR_UPDATE_FREQ);
    if (CY_RSLT_SUCCESS != cy_result)
    {
        printf("sensor timer set freq failed !\n");
    }
 	cyhal_timer_register_callback(&sensor_timer_obj, sensor_timer_callb, NULL);
    cyhal_timer_enable_event(&sensor_timer_obj, CYHAL_TIMER_IRQ_TERMINAL_COUNT, 3, true);
...
/*
 ...
 Function Description:
 @brief  This callback function is invoked on timeout of 100ms timer.
...
void mouse_timer_callb(void *callback_arg, cyhal_timer_event_t event)
{
	BaseType_t xHigherPriorityTaskWoken;
    xHigherPriorityTaskWoken = pdFALSE;
    vTaskNotifyGiveFromISR(mouse_task_handle, &xHigherPriorityTaskWoken);
    portYIELD_FROM_ISR(xHigherPriorityTaskWoken);
}
...
/*
 ...
 Function Description:
 @brief  This callback function is invoked on timeout of 50ms timer.
...
/*
 ...
 Function Description:
 @brief  This task updates dummy of mouse orientation value every time it is notified
         and sends a notification to the connected peer
...
{
	int8_t x, y, z;
	const uint16_t app_hids_mouse_report_len = (sizeof(app_hid_mouse_report));
printf("mouse task running\r\n");
	while(true)
    {
        ulTaskNotifyTake(pdTRUE, portMAX_DELAY);
        x++;
        ...
        if (bt_conn_id)
        {	
			app_hid_mouse_report[0] = 0;	// click
			app_hid_mouse_report[1] = x;	// xMotion
			app_hid_mouse_report[2] = y;	//yMotion
			app_hid_mouse_report[3] = z;	// scroll

            if (app_hids_mouse_in_report_client_char_config[0] & GATT_CLIENT_CONFIG_NOTIFICATION)
            {
                wiced_bt_gatt_server_send_notification(bt_conn_id,
                                                    HDLC_HIDS_MOUSE_IN_REPORT_VALUE,
                                                    app_hids_mouse_report_len,
                                                    app_hid_mouse_report,NULL);
                printf("\r\n================================================\r\n");
                printf( "Sending Notification: mouse : %i \t %i \t %i\r\n",
                        								x,	 y, 	z);
                printf("================================================\r\n");
            }
        }
...
/*
 ...
 Function Description:
 @brief  This task updates the sensor data while timer reach the counter
...
{
    /* Start the timer */
    if (CY_RSLT_SUCCESS != cyhal_timer_start(&mouse_timer_obj))
    {
        printf("mouse timer start failed !");
        CY_ASSERT(0);
    }

	if (CY_RSLT_SUCCESS != cyhal_timer_start(&sensor_timer_obj))
    {
        printf("sensor timer start failed !");
        CY_ASSERT(0);
    }
}
/**
...
void sensor_init()
{
	cy_rslt_t result;
	 /* Init SPI master */
   /* result = cyhal_spi_init(&mSPI, SPI_MOSI_PIN, SPI_MISO_PIN, SPI_CLK_PIN,
                            CS_SENSOR_1_PIN, NULL, BITS_PER_FRAME,
...
void sensor_update()
{
    ...
    poll_sensor(&mSPI, CS_SENSOR_1_PIN, sensor_rx_buf, SENSOR_REG_BYTES);
    parse_sensor_data(sensor_rx_buf, &sensor1);

    poll_sensor(&mSPI, CS_SENSOR_2_PIN, sensor_rx_buf, SENSOR_REG_BYTES);
    parse_sensor_data(sensor_rx_buf, &sensor2);

    poll_sensor(&mSPI, CS_SENSOR_3_PIN, sensor_rx_buf, SENSOR_REG_BYTES);
    parse_sensor_data(sensor_rx_buf, &sensor3);
}
